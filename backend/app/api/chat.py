...
from dynamodb.dynamodb_client import save_chat_history, get_chat_history, get_user_chat_histories
...
# Set up chat memory and system prompt
SYSTEM_PROMPT = (
    ...
    "yet informative. Use bullet points or numbered lists to break down complex topics into digestible steps."
)

# 🚨 Global per-user conversation store
user_conversations = {}
...
@router.post("/chatbot")
async def chatbot_endpoint(request: Request, body: ChatRequest):
    try:
        username = request.query_params.get("username")
        if not username:
            raise HTTPException(status_code=400, detail="Username missing in chatbot request.")
        ...
        if len(conversation_history) == 1:  # Only system prompt
            context_results = query_vector_store(body.query, n_results=3)
            if not context_results or not context_results.get("documents") or not context_results["documents"][0]:
                fallback = ("I couldn't find much information on that topic. "
                            "Try refining your query or check specialized job platforms.")
                return {"query": body.query, "response": fallback}
...
Answer:"""
            ...
            conversation_history.append(HumanMessage(content=body.query))
        
        assistant_reply = chat_model.invoke(conversation_history)
        conversation_history.append(assistant_reply)

        return {"query": body.query, "response": assistant_reply.content}
...
async def exit_chat(request: Request):
    try:
        ...
        conversation_history = user_conversations.get(username, [SystemMessage(content=SYSTEM_PROMPT)])
...
async def list_saved_chats(request: Request):
    try:
        ...
        chats = get_user_chat_histories(user_id=username)
        return {"saved_chats": chats}
    except Exception as e:
        logger.error(f"Error listing saved chats: {str(e)}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def resume_conversation(chat_id: str, request: Request):
    try:
        ...
        record = get_chat_history(user_id=username, chat_id=chat_id)
        if not record:
            raise HTTPException(status_code=404, detail="Conversation not found.")
